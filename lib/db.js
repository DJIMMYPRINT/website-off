...
// Shape only — no secret ever leaves this module. A wrong key length is the
// fastest way to spot a truncated paste; a bad scheme explains a fetch that
// throws with no status.
// A JWT's header and payload are base64, not encrypted — anyone holding the
// token can already read them. Reporting just the role and project ref (never
// the signature) answers the only question a 401 leaves open: is this the
// service_role key, and is it for this project? Remove once resolved.
export function keyClaims() {
  try {
    const [, payload] = KEY.split('.')
    const c = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'))
    return { role: c.role || null, ref: c.ref || null, expired: c.exp ? c.exp * 1000 < Date.now() : null }
  } catch {
    return { role: null, ref: null, expired: null, unreadable: true }
  }
}

export function dbShape() {
  return {
    urlValid: VALID_URL.test(URL_),
    urlSource: urlFromEnv ? 'env' : 'default',
