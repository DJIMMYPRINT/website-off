// Server-side Supabase access: order tracking, and the photo library
// (see lib/photos.js, which builds on the two helpers exported here).
//
...
// L'URL retenue après nettoyage et validation. Exportée pour que
// lib/photos.js compose ses URL signées sans redéfinir sa propre valeur
// de repli, qui divergerait de celle-ci au premier changement de projet.
export const SUPABASE_URL_RESOLU = URL_


// Exported so lib/photos.js can reach its own SECURITY DEFINER functions
// without duplicating credential handling: the service_role key is read
...
// Storage REST call, authenticated with the same service_role key.
// `chemin` is appended to /storage/v1/ — e.g.
// `object/catalogue-photos/2026/photo.jpg`. Returns the raw Response so
// the caller decides how to read it (JSON, bytes, or just the status).
export async function storageRequest(chemin, init = {}) {
  if (!dbConfigured) return null
  return fetch(`${URL_}/storage/v1/${chemin}`, {
    ...init,
    headers: {
      apikey: KEY,
      Authorization: `Bearer ${KEY}`,
