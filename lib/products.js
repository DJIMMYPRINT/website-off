...
// Unit price for one size of one colour. Almost every product has a single
// price; where a colour carries its own grid (the tote bag) that grid wins.
export function unitPrice(product, colorName, size) {
  const c = product.colors && product.colors.find(x => x.name === colorName)
  const p = c && c.prices && c.prices[size]
...
export function hasPriceGrid(product) {
  return Boolean(product.colors && product.colors.some(c => c.prices))
}

// Cheapest and dearest a single piece can cost, across colours and sizes.
export function priceRange(product) {
  if (!hasPriceGrid(product)) return [product.price, product.price]
  const all = []
  for (const c of product.colors) {
    for (const v of Object.values(c.prices || {})) all.push(v)
    if (!c.prices) all.push(product.price)
  }
